...
namespace BJVSExtension.Utilities
{
    class RegionFinder
    {
        ...
        public List<CodeRegin> FindAll()
        {
            ...
            Dictionary<int, int> methodDictionary = new Dictionary<int, int>();
            ...
            for (int spanIndex = 0; spanIndex < ClassificationSpans.Count; spanIndex++)
            {
                ...
                var endLine = span.Span.End.GetContainingLine();
                var endLineNumber = endLine.LineNumber;
                var endLineText = endLine.GetText();
                if (span.ClassificationType.Classification == ClassificationName.MethodName)
                {
                    methodDictionary.Add(startLineNumber, endLineNumber);
                }
#endif
                ...
                if (span.ClassificationType.Classification == ClassificationName.Punctuation)
                {
                    ...
                    if (index > -1)
                    {
                        ...
                        {
                            region = Regions.LastOrDefault(n => !n.Complete && n.RegionType == CodeRegionType.Method);
                            if (region != null)
                            {
                                region.Complete = true;
                                var endpoint = span.Span.Start + index + 1;
                    ...
                    if (index > -1)
                    {
                        ...
                        for (int checkIndex = spanIndex - 1; checkIndex > 0; checkIndex--)
                        {
                            ...
                            if (ClassificationName.IsMethod(ClassificationSpans[checkIndex].ClassificationType.Classification))
                            {
                                isNewMethod = true;
                                break;
                            }
                        ...
                        if (isNewMethod)
                        {
                            ...
                            var blockRegion = new CodeRegin(startPoint + index, CodeRegionType.Method, EditorFactory, BufferFactory);
